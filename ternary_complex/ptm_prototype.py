...
def react_mod(
    ...
    for products in product_rdmols:
        ...
        products_3D = []
        for product in products:
            product3d = rdkit.Chem.Mol(product)
            rdkit.Chem.AllChem.EmbedMolecule(product3d, rdkit.Chem.AllChem.ETKDG())
            rdkit.Chem.rdmolops.AssignStereochemistryFrom3D(product3d)
            products_3D.append(product3d)


        product_offmols = [Molecule.from_rdkit(product) for product in products_3D]
